...
const DemandMatrixByWeek = ({contracts, headers}) => {
        ...
        return (
          <tr key={contract.id}>
            ...
            {headers.map((date, index) => {
              // Calculate demand for the week
              let demand = 0;
              for (; i < contract.demand.length; i++) {
                let needDate = moment(contract.demand[i].needDate).format('YYYY-MM-DD');
                if (needDate >= currentWeek && needDate < moment(currentWeek).add(1, 'week').format('YYYY-MM-DD')) {
              ...
              if (!borderToggle && moment(date).format('YYYY-MM-DD') >= moment().format('YYYY-MM-DD')) {
...
export default DemandMatrixByWeek;
